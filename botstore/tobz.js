...
INGAT JANGAN JUAL SCRIPT Asuip Store BOT KEPADA ORANG LAIN!
INGIN PREMIUM? CHAT TOBZ!

Asuip Store BOT BOT V3
*/
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6285955372133@c.us'
        ...
        //addFilter(from)
        if (isCmd && !isGroupMsg) {console.log(color('[ CHAT ]'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname))}
        if (isCmd && isGroupMsg) {console.log(color('[ GROUP ]'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle))}
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Asuip Store BOT', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Asuip Store BOT!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Asuip Store BOT!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Asuip Store BOT!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Asuip Store BOT!', id)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 Asuip Store BOT*`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink Asuip Store BOT*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev Asuip Store BOT*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev Asuip Store BOT*`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev Asuip Store BOT*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev Asuip Store BOT*`, id)
            }
            break
        case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan Asuip Store BOT', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
            ...
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin Asuip Store BOT', id)  
...
${desc}`)
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Asuip Store BOT!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'play':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Asuip Store BOT!`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'listbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Asuip Store BOT!`, id)
                ...
                let nomre = 1
                    ...
                    break
			case prefix+'bcim': // KASIH CREDIT DONG KALO COPAS
				if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Asuip Store BOT`, id)
					bctxt = body.slice(4)
					txtbc = `*「 LEVI BROADCAST 」*\n\n${bctxt}`
					...
					if(quotedMsg && quotedMsg.type == 'image'){
						...
						for(let grupnya of semuagrup){
							var cekgrup = await tobz.getChatById(grupnya)
							if(!cekgrup.isReadOnly(grupnya)) tobz.sendText(grupnya, txtbc)
						}
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Asuip Store BOT`, id)
                bctxt = body.slice(4)
                txtbc = `*「 Asuip Store BOT BROADCAST 」*\n\n${bctxt}`
                const semuagrup = await tobz.getAllChatIds();
                if(quotedMsg && quotedMsg.type == 'image'){
                    const mediaData = await decryptMedia(quotedMsg)
                    const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN Asuip Store BOT
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Asuip Store BOT', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Asuip Store BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Asuip Store BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Asuip Store BOT', id)
            await tobz.sendText(from,'Asuip Store BOT DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan Asuip Store BOT`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan Asuip Store BOT`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Asuip Store BOT atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'Asuip Store BOTadmin':
            let admn = `This is list of Asuip Store BOT Admin\nTotal : ${adminNumber.length}\n`
        ...
        case prefix+'limit':
            ...
            break
        case prefix+'gift': // Hanya Admin & Owner Asuip Store BOT yang bisa gift Limit
            if (!isAdmin, !isOwner) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Asuip Store BOT!`, id)
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Asuip Store BOT!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Asuip Store BOT!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Asuip Store BOT!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Asuip Store BOT!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Asuip Store BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Asuip Store BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Asuip Store BOT!`, id)
	...
	//list menu
        case prefix+'runtime':
            tobz.reply(from, `Asuip Store BOT telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
        ...
        case prefix+'groupbot':
            tobz.reply(from, `Link Group Asuip Store BOT : [ link group mu ]\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Asuip Store BOT', id)
		...
		//ferlito
		case prefix+'list':
			tobz.reply(from, listbeli())
			break
		case prefix+'beli':
			tobz.reply(from, belibang())
			break
        case prefix+'pesan':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (args.length === 1) return tobz.reply(from, `[❗] Kirim perintah *${prefix}pesan [teks]*\ncontoh : *${prefix}pesan formulir yang kamu isi*`)
            const pesanan = body.slice(11)
            if(!pesanan) return
            if(isGroupMsg){
                tobz.sendText(ownerNumber, `*[ ORDERAN BARU ]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\nGroup : ${formattedTitle}\n\n${pesanan}`)
                tobz.reply(from, '*Selamat Orderan anda telah tersampaikan ke owner kami*\n*Silahkan melakukan transfer ke pembayaran yang tersedia*\n*Kami akan mengirim pesan ke nomermu jika orderan telah di buat, tunggu beberapa menit setelah pembayaran selesai !*\nlaporan palsu/main2 tidak akan ditanggapi.' ,id)
            }else{
                tobz.sendText(ownerNumber, `*[ ORDERAN BARU ]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\n\n${pesanan}`)
                tobz.reply(from, '*Selamat Orderan anda telah tersampaikan ke owner kami*\n*Silahkan melakukan transfer ke pembayaran yang tersedia*\n*Kami akan mengirim pesan ke nomermu jika orderan telah di buat, tunggu beberapa menit setelah pembayaran selesai !*\nlaporan palsu/main2 tidak akan ditanggapi.', id)
            }
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Asuip Store BOT, Link Ada Di Bio atau Bisa Mengetik #Asuip Store BOTgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
